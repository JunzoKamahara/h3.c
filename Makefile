...
h3_ref2va_offline_test: tests/test_ref2va_offline.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)

h3_job_test: tests/test_h3_job.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)

h3_video_job_test: tests/test_h3_video_job.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)

h3_mem_test: tests/test_h3_mem.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)

h3_video_http_test: tests/test_h3_video_http.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)

h3_media_tools_test: tests/test_h3_media_tools.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)

h3_sched_probe_test: tests/test_h3_sched_probe.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)

h3_ttft_probe_test: tests/test_h3_ttft_probe.o $(LIB_OBJ)
	$(CC) -o $@ $^ $(LDLIBS)
...
# QINT-015h-2b-2: draft prefix K/V + prime/propose/sync lifecycle.
spec-eagle-prefix-check: h3_qwen_spec_test
...
# P10-REF2VA-00 check: swapping the Ref2VA reference image measurably changes
# generated output. Slow (streams the 62 GB Ref2VA transformer twice); not
# part of `make test`.
...
# P8-VID-01: one real video job through the manager + a concurrent chat decode.
# Slow (loads the FL2VA transformer + VAEs); not part of `make test`.
...
# 409-before / MP4-after on /content, chat still answers mid-job. Slow; not
# part of `make test` (phase4-check covers the routing without generating).
p8-vid-http-check: h3_video_http_test
...
# P8-SCHED-01a: GPU-cadence probe -- block timing + does a per-block yield let
# a concurrent chat through. No scheduler; generation output unchanged. Slow.
...
clean:
	rm -f h3 h3_serve h3_tests h3_metal_tests h3_bf16_tests h3_tokenizer_tests \
		...
		h3_qwen_kv_test h3_qwen_chat_test h3_qwen_server_test h3_p8_img_test h3_job_test h3_video_job_test h3_mem_test h3_video_http_test h3_media_tools_test h3_sched_probe_test h3_ttft_probe_test h3_ref2va_offline_test h3_qwen_tools_test h3_qwen_responses_test h3_qwen_stream_test h3_qwen_vlm_test h3_qwen_bench \
