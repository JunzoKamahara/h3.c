...
int qwen_kv_eval(struct qwen_session *session, const uint32_t *token_ids,
    ...
    for (int layer = 0; layer < QWEN_LM_TOTAL_LAYERS; layer++) {
        if (!qwen_layer_weights_load(kv->store, gpu, layer, &weights, error,
                                     error_size))
            goto done;

        int prep_ok =
            gpu_ok(gpu, h3_gpu_begin(gpu), error, error_size, "prep begin") &&
            qwen_layer_prep(gpu, &weights, m, s.hidden, s.norm, s.query,
                            s.key_new, s.value_new, s.rope_cos, s.rope_sin,
        ...
        int body_ok =
            ...
            qwen_layer_finish(gpu, &weights, m, s.hidden, s.attention_heads,
                              s.norm, s.attention_output, s.gate, s.up,
